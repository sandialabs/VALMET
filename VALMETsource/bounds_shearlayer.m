function [flower,fupper, flower_n,fupper_n,cc_l,cc_u,sse_l,sse_u] = ...
        bounds_shearlayer(c0min,c0max,xpoints, xi,yi,sse, tolcon,tolfun)
%  procedure to find upper and lower bounds on a regression: 
%    y(x) = 1 + c1 * (1/(1 + c2*x^c3) -1) 
%
%  Initial guesses for c1,c2,c3 are in arrays c0min and c0max.
...
%               c0min and c0max.    
%   5/27/07 HJI If nipl = 2 and c0min(2,:) = [0,0,0], then clower0(2,:) will
...
for i = 1:np;
   for j = 1:nipl
      ...
      while isnan(flower_nip(j)) || isnan(sse_l(j+(i-1)*nipl)) || flag == 0
         [c_lower,fval,flag] = fmincon(@(c) form_shearlayer(c,xpoints(i)), ...
             clower0(j,:), [],[],[],[],[],[], ...
             @(c) form_shearlayer_constraint(c,xi,yi,sse),options);
         flower_nip(j) = form_shearlayer(c_lower,xpoints(i));
         sse_l(j+(i-1)*nipl) = sumsqerr_shearlayer(c_lower,xi,yi);
         if retry_count < 10
...
%      lower_exit_flag = flag
   ...
   for j = 1:nipu
      ...
      while isnan(fupper_nip(j)) || isnan(sse_u(j+(i-1)*nipu)) || flag == 0
         [c_upper,fval,flag] = fmincon(@(c) negform_shearlayer(c,xpoints(i)), ...
             cupper0(j,:), [],[],[],[],[],[], ...
             @(c) form_shearlayer_constraint(c,xi,yi,sse),options);
         fupper_nip(j) = form_shearlayer(c_upper,xpoints(i));
         sse_u(j+(i-1)*nipu) = sumsqerr_shearlayer(c_upper,xi,yi);          
         if retry_count < 10
