function [flower,fupper, flower_n,fupper_n,cc_l,cc_u,sse_l,sse_u] = ...
        bounds110(c0min,c0max,xpoints, xi,yi,sse, tolcon,tolfun)
...
for i = 1:np;
   for j = 1:nipl
      ...
      while isnan(flower_nip(j)) || isnan(sse_l(j+(i-1)*nipl)) || flag == 0
         [c_lower,fval,flag] = fmincon(@(c) form110(c,xpoints(i)), ...
             clower0(j,:), [],[],[],[],[],[], ...
             @(c) form110_constraint(c,xi,yi,sse),options);
         flower_nip(j) = form110(c_lower,xpoints(i));
         sse_l(j+(i-1)*nipl) = sumsqerr110(c_lower,xi,yi);
         if retry_count < 10
...
%      lower_exit_flag = flag
   ...
   for j = 1:nipu
      ...
      while isnan(fupper_nip(j)) || isnan(sse_u(j+(i-1)*nipu)) || flag == 0
         [c_upper,fval,flag] = fmincon(@(c) negform110(c,xpoints(i)), ...
             cupper0(j,:), [],[],[],[],[],[], ...
             @(c) form110_constraint(c,xi,yi,sse),options);
         fupper_nip(j) = form110(c_upper,xpoints(i));
         sse_u(j+(i-1)*nipu) = sumsqerr110(c_upper,xi,yi);          
         if retry_count < 10
