function [flower,fupper, flower_n,fupper_n,cc_l,cc_u,sse_l,sse_u] = ...
        bounds_lorentzian(c0min,c0max,xpoints, xi,yi,sse, tolcon,tolfun)
%  procedure to find upper and lower bounds on a regression: 
%    y(x) = c1 + c2/(1. + ((x - c3)/c4)^2)
%
%  Initial guesses for c1,c2,c3, and c4 are in arrays c0min and c0max.
...
for i = 1:np;
   for j = 1:nipl
      ...
      while isnan(flower_nip(j)) || isnan(sse_l(j+(i-1)*nipl)) || flag == 0
         [c_lower,fval,flag] = fmincon(@(c) form_lorentzian(c,xpoints(i)), ...
             clower0(j,:), [],[],[],[],[],[], ...
             @(c) form_lorentzian_constraint(c,xi,yi,sse),options);
         flower_nip(j) = form_lorentzian(c_lower,xpoints(i));
         sse_l(j+(i-1)*nipl) = sumsqerr_lorentzian(c_lower,xi,yi);
         if retry_count < 10
...
%      lower_exit_flag = flag
   ...
   for j = 1:nipu
      ...
      while isnan(fupper_nip(j)) || isnan(sse_u(j+(i-1)*nipu)) || flag == 0
         [c_upper,fval,flag] = fmincon(@(c) negform_lorentzian(c,xpoints(i)), ...
             cupper0(j,:), [],[],[],[],[],[], ...
             @(c) form_lorentzian_constraint(c,xi,yi,sse),options);
         fupper_nip(j) = form_lorentzian(c_upper,xpoints(i));
         sse_u(j+(i-1)*nipu) = sumsqerr_lorentzian(c_upper,xi,yi);          
         if retry_count < 10
